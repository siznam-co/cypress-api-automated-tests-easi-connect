...
describe("Claims Endpoints", () => {
    before(() => {
        ...
        cy.MemberReferrals(Cypress.env("member_ID_1")).then(count => {
            window.MemberReferralsCount = count
        }) // Note the MemberReferrals before creating new.
    ...
    after(() => {
        ...
        cy.MemberReferrals(Cypress.env("member_ID_1")).then(count => {
            expect(window.MemberReferralsCount).be.equal(count - 1)
        })
    })
})